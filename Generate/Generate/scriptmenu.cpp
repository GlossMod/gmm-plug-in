...
{
	GRAPHICS::DRAW_RECT((lineLeft + (lineWidth * 0.5f)), (lineTop + (lineHeight * 0.5f)), lineWidth, lineHeight, r, g, b, a, 0);
}
...
{	
	...
	HUD::SET_TEXT_COLOUR(color.r, color.g, color.b, static_cast<int>(color.a / 1.1f));
	HUD::SET_TEXT_CENTRE(0);
	HUD::SET_TEXT_DROPSHADOW(0, 0, 0, 0, 0);
	DrawText(lineLeft + lineWidth - lineWidth / 6.35f, lineTop + lineHeight / 4.8f, GetState() ? "[¿ª]" : "[¹Ø]");
}
...
void MenuItemMenu::OnDraw(float lineTop, float lineLeft, bool active)
{
	...
	ColorRgba color = active ? GetColorTextActive() : GetColorText();
	HUD::SET_TEXT_SCALE(0.0, lineHeight * 8.0f);
	HUD::SET_TEXT_COLOUR(color.r, color.g, color.b, color.a / 2);
	HUD::SET_TEXT_CENTRE(0);
	HUD::SET_TEXT_DROPSHADOW(0, 0, 0, 0, 0);
